...
2. Inspect any subcommand with `newton <command> --help` to view supported flags.
3. Refer to the reference sheets below for typical workflows, required arguments, and example invocations.
4. Adjust paths, project IDs, and formats to match your workspace before executing a command.
...
3. **newton batch PROJECT_ID**: Consume queued plans with hooks and automation. See [references/batch.md](references/batch.md).
...
cargo run -- batch my-project --workspace ./workspace --once
